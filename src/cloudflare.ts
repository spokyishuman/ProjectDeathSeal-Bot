...
export class CloudflareBypassClient {
    private static axiosInstance: AxiosInstance;

    static configure(): void {
        this.axiosInstance = axios.create({ timeout: 30000, validateStatus: () => true });
        ...
        this.axiosInstance.interceptors.response.use(
            ...
            async (error) => {
                ...
                if ([403, 429, 503, 520, 521, 522, 524].includes(error.response?.status) && !config._retry) {
                    config._retry = true; config._retryCount = (config._retryCount || 0) + 1;
    ...
    static async get(url: string, config?: AxiosRequestConfig): Promise<any> { return this.axiosInstance.get(url, config); }
    static async post(url: string, data?: any, config?: AxiosRequestConfig): Promise<any> { return this.axiosInstance.post(url, data, config); }
    static getInstance(): AxiosInstance { if (!this.axiosInstance) this.configure(); return this.axiosInstance; }
}
